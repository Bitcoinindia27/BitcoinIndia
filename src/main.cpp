...
#if defined(NDEBUG)
# error "BitcoinIndia cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xbff10cdedae75d7c495c09b7260c5e191443fdf9b42e22afab60fb681c3cb882");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // BitcoinIndia: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "BitcoinIndia Signed Message:\n";
...
{
    // BitcoinIndia: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // BitcoinIndia: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // BitcoinIndia: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1505996714, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "045000";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("044f920ac5949bea86130b8d1c22c94a42443e879bebd11b132919ce5e1d33d6cb8a6fb49963847e071d2e1e72a9e7ebe8edcf2efcbca3aca6366983dbc1957034") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1505996714;
        ...
        assert(block.hashMerkleRoot == uint256("0x9f8929a5b2c9ee724ff15d0975b129ded50044d3812040537233c1bf4c478ac9"));
...
unsigned char pchMessageStart[4] = { 0xbe, 0x52, 0x62, 0x30 }; // BitcoinIndia: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitcoinIndiaMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitcoinIndiaMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitcoinIndiaMiner(CWallet *pwallet)
{
    printf("BitcoinIndiaMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bitcoinindia-miner");
    ...
    try { loop {
        ...
        printf("Running BitcoinIndiaMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BitcoinIndiaMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitcoinIndiaMiner, pwallet));
}
